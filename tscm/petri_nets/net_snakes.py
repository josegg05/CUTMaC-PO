from tscm.petri_nets import tpn
...
def net_snakes_create(petri_net):
    ...
    for x in range(len(petri_net.transitions) - 1):
        # print(x)
        petri_snake.add_transition(
            Transition(petri_net.transitions[x+1][0], min_time=petri_net.transitions[x+1][4],
                       max_time=petri_net.transitions[x+1][5]))     # "Transitions"

        #if petri_net.transitions[x+1][6] != "NaN":  # "arcs_in"
        arcs_in_t = list(petri_net.transitions[x+1][6])     # "arcs_in"
        for y in range(len(arcs_in_t)):
            if "*" not in arcs_in_t[y]:
                petri_snake.add_input(arcs_in_t[y], petri_net.transitions[x+1][0], Value(dot))
        arcs_out_t = list(petri_net.transitions[x+1][7])    # "arcs_out"
